...
The core logic, defined in `src/agent/index.ts`, showcases a straightforward chatbot that responds to user queries while maintaining context from previous messages.
...
2. **Extend the index**: The core logic of the chatbot is defined in [index.ts](src/agent/index.ts). You can modify this file to add new nodes, edges, or change the flow of the conversation.
...
While iterating on your index, you can edit past state and rerun your app from previous states to debug specific nodes. Local changes will be automatically applied via hot reload. Try experimenting with:

- Modifying the system prompt to give your chatbot a unique personality.
- Adding new nodes to the index for more complex conversation flows.
